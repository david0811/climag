...
def PlotAgVar(data, model, agvar, metric, vmin, vmax, cmap, save=False):
    """
    ...
    ax.set_title(model + ' ' + agvar + ' ' + metric, fontsize=20)
    plt.tight_layout()
    plt.show()


def PlotYield(data, model, metric, vmin, vmax, cmap, save=False):
    """
    ...
    coast.geometry.boundary.plot(ax=ax, linewidth=0.5, edgecolor="black", color=None)
    ax.tick_params(labelbottom=False, labelleft=False)
    ax.set_title(model + ' ' + metric, fontsize=20)
    plt.tight_layout()
    plt.show()
